...
def _font(size: int) -> ImageFont.FreeTypeFont:
    ...
    raise RuntimeError("未找到用于生成 Linux ARM64 OCR 测试图片的字体。")


def main() -> int:
    parser = argparse.ArgumentParser(description="Create a synthetic Linux ARM64 OCR input")
    ...
    title = _font(66)
    body = _font(50)
    draw.text((100, 100), f"{args.platform_label} PP-OCRv6 CLI Test", fill="#111827", font=title)
