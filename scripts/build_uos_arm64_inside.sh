...
BUILD_ROOT="$ROOT/build/uos-arm64"
...
CHECKSUM_PATH="$ROOT/dist/SHA256SUMS-uos-arm64-$EDITION.txt"
BUILD_INFO="$BUILD_ROOT/metadata/$EDITION/build-info.json"

if [ "$(uname -s)" != "Linux" ] || [ "$(uname -m)" != "aarch64" ]; then
  echo "[error] 请在 UOS/Linux ARM64（aarch64）中执行本脚本。" >&2
  exit 2
fi
if [ ! -x "$PYTHON" ]; then
...
"$PYTHON" "$ROOT/scripts/write_build_info.py" "$BUILD_INFO" \
  --edition "$EDITION" --version "$VERSION" --platform uos-v20 --architecture arm64
...
"$PYTHON" "$ROOT/scripts/write_package_readme.py" "$PACKAGE_ROOT/使用说明.txt" \
  --platform uos-arm64 --edition "$EDITION"
chmod +x \
...
for required_path in "${REQUIRED_PORTABLE_PATHS[@]}"; do
  if [ ! -e "$required_path" ]; then
    echo "[error] UOS ARM64 绿色版缺少必要文件：$required_path" >&2
    exit 1
  fi
done
if [ -d "$PACKAGE_ROOT/app" ]; then
  echo "[error] UOS ARM64 绿色版不应包含 app 子目录。" >&2
  exit 1
fi
if find "$PACKAGE_ROOT" -maxdepth 1 -type f -name '启动*.sh' | grep -q .; then
  echo "[error] UOS ARM64 绿色版不应依赖启动脚本。" >&2
  exit 1
fi
...
if [[ "$VERSION_OUTPUT" != *"($EDITION, uos-v20, arm64)"* ]]; then
...
if [ ! -s "$TEST_ROOT/gui.png" ]; then
  echo "[error] UOS ARM64 图形界面未生成冒烟测试截图。" >&2
  exit 1
fi
...
# reported by Linux desktop users.
...
if [[ "$UNICODE_VERSION_OUTPUT" != *"($EDITION, uos-v20, arm64)"* ]]; then
...
if [ ! -s "$TEST_ROOT/gui-unicode-path.png" ]; then
  echo "[error] UOS ARM64 图形界面未通过中文路径启动测试。" >&2
  exit 1
fi

SMOKE_IMAGE="$TEST_ROOT/uos-arm64-ocr.png"
SMOKE_DOCX="$TEST_ROOT/uos-arm64-office.docx"
"$PYTHON" "$ROOT/scripts/create_kylin_arm64_smoke_input.py" \
  "$SMOKE_IMAGE" --platform-label "UOS ARM64"
"$PYTHON" "$ROOT/scripts/create_office_smoke_input.py" \
  "$SMOKE_DOCX" --platform-label "UOS ARM64"

"$MAIN_EXECUTABLE" --cli \
  "$SMOKE_IMAGE" -o "$TEST_ROOT/ocr-output" --no-table
if ! grep -R -q "UOS ARM64" "$TEST_ROOT/ocr-output"; then
  echo "[error] ARM64 冻结程序没有识别出 UOS ARM64。" >&2
  exit 1
fi
...
if report.get("ocr_blocks", 0) < 1 or report.get("tables", 0) < 1:
    raise SystemExit(f"UOS ARM64 table smoke failed: {report}")
...
if ! grep -R -q "UOS ARM64 LibreOffice bundled conversion test" "$TEST_ROOT/office-output"; then
